...
machine:
  ...
  kubelet:
    ...
    extraConfig:
      ...
      maxPods: 512
  sysctls:
    {{- if and .Values.nr_hugepages (gt .Values.nr_hugepages 0) }}
  ...
  install:
    ...
    {{- if .Values.installDisk }}
    {{- $installDisk := .Values.installDisk }}
    {{- range (lookup "disks" "" "").items }}
    {{- if eq .spec.model $installDisk }}
    disk: /dev/disk/by-id/nvme-{{ .spec.wwid }}
    {{ break }}
    {{- end }}
    {{- end }}
    {{- else }}
